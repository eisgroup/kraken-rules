...
public class LiteralsBenchmark {
    ...
    public void setup() {
        ...
        for (int i = 0; i < DEFAULT_UNIQUE_EXPRESSIONS; i++) {
            String expression = i % 2 == 0 ? "'string" + i + "'" : Integer.toString(i);
            this.expressions[i] = (expression);

            AcceleratedExpressionEvaluator.expressionCache.put(expression,
                    new ExpressionMetadata(expression, AstType.LITERAL, i % 2 == 0 ? "string" + i : Integer.valueOf(i)));
        }
    }

    @Benchmark
    public void literalsRegular(Blackhole bh) {
    ...
    @Benchmark
    public void literalsAccelerated(Blackhole bh) {
        Map<String, Object> empty = Collections.emptyMap();
        for(int i = 0; i < expressions.length; i++) {
