...
import { EntryPointResult, RuleEvaluationResults, FieldEvaluationResult } from 'kraken-engine-api'
...
export const reduceResult = (entryPointResult: EntryPointResult): ReducedResults => {
    const allRuleResults = entryPointResult.getAllRuleResults().sort((first, second) => {
    ...
    const byKrakenInfo = (first: ResultErrorMessage, second: ResultErrorMessage) => {
        if (
            first.ruleName === second.ruleName &&
    ...
    return {
        ...
        fieldResults: fieldResultsWithoutExternalReferences as unknown as Record<string, FieldEvaluationResult>[],
