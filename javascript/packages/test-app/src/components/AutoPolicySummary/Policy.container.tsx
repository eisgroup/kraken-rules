...
import { KRAKEN_FUNCTIONS_POLICY as policyFunctions } from 'kraken-test-product-model-tree'
...
import {
    ...
    DimensionSetBundleCache,
    ExpressionContextManagerImpl,
...
import { NavigationStepsContextInstanceInfoResolver } from '../../rule-engine/spi/navpath/NavigationStepsContextInstanceInfoResolver'
...
const evaluationConfig = (data: domain.Policy) => ({ currencyCd: 'USD', context: { dimensions: getDimensions(data) } })
...
export class AutoPolicySummaryContainer extends React.Component<unknown, AutoPolicyViewState & EvaluationResponse> {
    engine: SyncEngine
    cache = new DimensionSetBundleCache(
    ...
    async componentDidMount(): Promise<void> {
        ...
        const bundlePromises = Object.values(entryPointNames)
            ...
            .map(name =>
                this.fetchBundle(name).then(entryPointBundle =>
                    this.cache.add(name, getDimensions(this.state.autoPolicy), entryPointBundle),
                ),
            )
        ...
        this.engine = new SyncEngine({
            ...
            modelTree: modelTree as ContextModelTree.ContextModelTree,
            functions: policyFunctions as KelFunction[],
        })
        this.setState({ isLoading: false })
    }
    fetchBundle = (entryPointName: string) => {
        return fetch.bundle(entryPointName, false, getDimensions(this.state.autoPolicy))
    }
    fetchNewRulesForEntryPoint = async (entryPointName: string) => {
        this.cache.clearCache()
        return this.fetchBundle(entryPointName).then(entryPointBundle => {
            this.cache.add(entryPointName, getDimensions(this.state.autoPolicy), entryPointBundle)
            this.cache.setExpressionContext(entryPointBundle.expressionContext)
        })
    }
    ...
    validate = async () => {
        ...
        const entryPointResult = this.engine.evaluate(
            this.state.autoPolicy,
            entryPointName,
            evaluationConfig(this.state.autoPolicy),
        )
        const { allRuleResults, fieldResults, validationStatus } = reduceResult(entryPointResult)
        ...
        try {
            rawResult = await fetch.raw(this.deepCopy(modelBeforeEvaluation), entryPointName)
            defaultResult = {}
            validationResult = await fetch.validations(this.deepCopy(modelBeforeEvaluation), entryPointName)
    ...
    deepCopy = (objectToCopy: object) => JSON.parse(JSON.stringify(objectToCopy))
    onClickResetUI = () => {
        this.setState({
            backendRawResult: {},
    ...
    onCreditCardInfoChange = (creditCardInfo: domain.CreditCardInfo) => {
        const autoPolicy = {
            ...this.state.autoPolicy,
            billingInfo: { ...this.state.autoPolicy.billingInfo, creditCardInfo },
        }
        this.setState({ autoPolicy })
    }
    ...
    onVehicleChange = (changedVehicleIndex: number) => (vehicle: domain.Vehicle) => {
        const autoPolicy = {
            ...this.state.autoPolicy,
            riskItems: this.state.autoPolicy.riskItems.map((riskItem, index) =>
                index === changedVehicleIndex ? { ...vehicle } : riskItem,
            ),
        }
        this.setState({ autoPolicy })
    }
