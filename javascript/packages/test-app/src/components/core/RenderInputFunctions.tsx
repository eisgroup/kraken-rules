...
export const renderers = {
    ...
    input: (config: InputRendererProps<string>) => (
        <Input onChange={(e: React.FormEvent<HTMLInputElement>) => config.onChange(e)} {...config} />
    ),
    inputDisabled: (config: InputRendererProps<string>) => (
        <Input {...config} disabled onChange={(e: React.FormEvent<HTMLInputElement>) => config.onChange(e)} />
    ),
    inputNumber: (config: InputRendererProps<number>) => <InputNumber {...config} />,
    inputCurrency: (config: InputRendererProps<number>) => (
        <InputNumber formatter={value => `$ ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')} {...config} />
    ),
