...
export class RuleCreator extends React.PureComponent<RuleCreatorProps, State> {
    ...
    onAssertChange(e: React.ChangeEvent<HTMLTextAreaElement>): void {
        this.setState({ assertExpression: e.target.value })
    }

    onDefaultChange(e: React.ChangeEvent<HTMLTextAreaElement>): void {
        this.setState({ defaultExpression: e.target.value })
    }

    onFullChange(e: React.ChangeEvent<HTMLTextAreaElement>): void {
    ...
    createFullRule(): void {
        ...
        fetch
            .createRule(this.state.fullRule)
            .then(() => {
                    ...
                    <Col lg={20} md={20}>
                        ...
                        <h5>
                            ...
                            {!this.state.defaultLoading ? (
                                <a onClick={this.createDefault}>&nbsp;(submit)</a>
                            ) : (
                                ' (submitting...)'
                            )}
                        </h5>
                        ...
                        <h5>
                            ...
                            {!this.state.fullLoading ? (
                                <a id={`createrule-full-submit-${id}`} onClick={this.createFullRule}>
                                    &nbsp;(submit)
                                </a>
                            ) : (
                                ' (submitting...)'
