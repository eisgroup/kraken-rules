...
export const ContextDefinitionInfo = React.memo((props: { contextName: string }) => {
    ...
    return (<div className="ContextDefinitionInfo">
        <h2>{props.contextName}</h2>
        {Object.keys(children).length ? <React.Fragment>
            <h4>Children</h4>
            <div>{Object.keys(children).map(p => <Tag key={p} style={tagStyle} >{p}</Tag>)}</div>
        </React.Fragment> : NULL
        }
        {Object.keys(projection).length ? <React.Fragment>
            ...
            <div>
                {Object
                    ...
                    .map(p => {
                        ...
                        const tag = (<Tag color={isChildAndField
                            ? "orange" :
                            isField ? "blue" : undefined
                        }
                            style={tagStyle}>
                            {p}
                        </Tag>);
                ...
                <div>
                    {inheritance.map(p => (
                        <Tooltip
                            key={p}
                            placement="bottom"
                            title={"Fields: " + resolveFieldNames(p).join(" | ")}>
