...
const pom = xmlParser.parse(fs.readFileSync(pomPath, { encoding: 'utf8' }))

console.log(`  version is: '${pom.project.version}'`)

const genFolderPath = path.join(__dirname, 'src', 'engine', 'generated')

if (!fs.existsSync(genFolderPath)) {
    fs.mkdirSync(genFolderPath)
}

const genPath = path.join(__dirname, 'src', 'engine', 'generated', 'engineVersion.ts')
