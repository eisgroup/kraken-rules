...
describe('Engine Expressions Sanity Test', () => {
    ...
    it('should Default Dates With Nested Functions', () => {
        ...
        expect(results).k_toHaveExpressionsFailures(0)
        expect(policy.transactionDetails.txCreateDate).k_toBeDateEqualTo(new Date('2022-05-05'))
        expect(policy.termDetails.termEffectiveDate).k_toBeDateEqualTo(new Date('2019-06-06'))
    })
    ...
    it('should Default With Filter Count Result', () => {
        ...
        policy.parties = [{ id: '15', cd: 'Party', driverInfo: { id: '14', cd: 'DriverInfo' } }]
        ...
        policy.riskItems = [
            {
                id: '1',
                cd: 'Vehicle',
                collCoverages: [{ id: '11', cd: 'COLLCoverage', limitAmount: 10 }],
    ...
    it('shouldEvaluateExpressionsWithNullSafeOperator', () => {
        const policy = empty()
        const results = sanityEngine.evaluate(policy, 'Expressions_nullsafe')
        const fieldResults = new FieldMetadataReducer().reduce(results)
    ...
    it('shouldEvaluateExpressionsWithVariables_positive', () => {
        const policy = empty()
        policy.state = 'CA'
        policy.riskItems = [
            {
        ...
        const results = sanityEngine.evaluate(policy, 'Expressions_variables')
        const fieldResults = new FieldMetadataReducer().reduce(results)
    ...
    it('shouldEvaluateExpressionsWithVariables_negative', () => {
        const policy = empty()
        policy.state = 'CA'
        policy.riskItems = [
            {
    ...
    it('shouldEvaluateExpressionsWithEscapes', () => {
        const policy = empty()

        const results = sanityEngine.evaluate(policy, 'Expressions_escapes')

        expect(results).k_toHaveExpressionsFailures(0)
        expect(policy.state).toBe('"\'\'"\\\\')
    })
})
