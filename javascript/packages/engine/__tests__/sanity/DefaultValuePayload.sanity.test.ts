...
describe("Engine Sanity DefaultValue Payload Test", () => {
    ...
    it("should execute 'InitAutoPolicy' entrypoint with empty data", () => {
        ...
        expect(dataObject.transactionDetails!.txEffectiveDate)
            .k_toBeDate(new Date("2018-04-30T10:04:00Z"));
        expect(dataObject.transactionDetails!.txCreateDate)
            .k_toBeTodayDate();
        expect(dataObject.accessTrackInfo!.createdOn)
            .k_toBeTodayDate();
        expect(dataObject.accessTrackInfo!.updatedOn)
            .k_toBeTodayDate();
        expect(dataObject.accessTrackInfo!.updatedBy)
            .toContain("qa2");
        expect(dataObject.termDetails!.termEffectiveDate)
            .k_toBeTodayDate();
    });
    it("should execute 'InitAutoPolicy' entrypoint with valid data", () => {
        ...
        expect(dataObject.transactionDetails!.txEffectiveDate).k_toBeDate(new Date("2018-01-01T00:00:00Z"));
        expect(dataObject.transactionDetails!.txCreateDate).k_toBeDate(new Date("2018-01-01"));
        expect(dataObject.accessTrackInfo!.createdOn).k_toBeDate(new Date("2000-01-01"));
        expect(dataObject.termDetails!.termEffectiveDate).k_toBeDate(new Date("2000-01-01"));
        expect(dataObject.termDetails!.termEffectiveDate).k_toBeDate(new Date("3000-01-01"));
    ...
    it("should throw an error when two rules defined on one field", () => {
        ...
        expect(() => sanityEngine.evaluate(data, "InitAutoPolicy")).toThrow("On field 'Policy:0:state' "
            + "evaluated '2' default rules: 'R0005, R0005a'. One default rule per field can be evaluated."
        );
    });
});
