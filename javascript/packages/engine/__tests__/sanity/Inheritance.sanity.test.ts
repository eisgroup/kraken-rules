...
describe('Sanity Coverages Inheritance tests', () => {
    ...
    describe('Inheritance field test on VisibilityCarCoverage', () => {
        it("should execute 'VisibilityCarCoverage' entry point", async () => {
            ...
            expect(results).k_toMatchResultsStats({ total: 7, disabled: 0, hidden: 7 })
            expect(results).k_toHaveExpressionsFailures(0)
            expect({ entryPointResults: results, fieldResults }).k_toMatchResultsSnapshots()
        })
    })
    ...
    it('shouldEvaluateWithoutDuplicatedResults', async () => {
        ...
        const results = sanityEngine.evaluate(policy, 'EvaluateWithoutDuplicatedResults')
