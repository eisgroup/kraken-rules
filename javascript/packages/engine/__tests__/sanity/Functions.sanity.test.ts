...
describe('Engine Functions Sanity Test', () => {
    ...
    it('should execute kel function', () => {
        const policy = empty()
        policy.riskItems = [
            {
                id: 'vehicle1',
        ...
        const results = sanityEngine.evaluate(policy, 'Expressions_kel_functions')
        expect(results).k_toMatchResultsStats({ total: 2, critical: 0, warning: 0, info: 0 })
        expect(results).k_toHaveExpressionsFailures(0)
