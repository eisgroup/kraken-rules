...
describe('Functions registry', () => {
    ...
    it('should add function', () => {
        ...
        expect(Object.entries(r.registeredFunctions())).toHaveLength(length + 1)
    })
    ...
    it('should fail on adding undefined or null as a function name', () => {
        const addU = () =>
            r.add({
                // @ts-expect-error testing ngative case
                name: undefined,
                function: Math.abs,
            })
        const addN = () =>
    ...
    describe('createInstanceFunctions', () => {
        const functions = FunctionRegistry.createInstanceFunctions(mock.spi.dataResolver, (name: string) => {
