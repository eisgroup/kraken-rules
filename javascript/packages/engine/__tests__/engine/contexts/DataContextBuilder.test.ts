...
describe("DataContextBuilder", () => {
    ...
    it("should build from extracted array first element", () => {
        ...
        (party as any).externalReferences = {};
        expect(party).toMatchSnapshot();
    });
    it("should build from extracted object", () => {
        const parent: DataContext = builder.buildFromRoot(dataObject);
        const human0DataContext =
            builder.buildFromExtractedObject(dataObject.insured!, Insured.name, parent, 0);
        const dataContext =
            builder.buildFromExtractedObject(dataObject.insured!.addressInfo!, AddressInfo.name, human0DataContext);
        (dataContext as any).externalReferences = {};
    ...
    it("should throw on empty info fields", () => {
        const parent: DataContext = builder.buildFromRoot(dataObject);
        expect(() => builder
            .buildFromExtractedObject({}, "Policy", parent, 2))
            .toThrowError();
    });
});
