...
describe("lengthPayloadHandler", () => {
    ...
    it("should return payload payloadResult with type LengthPayloadResult", () => {
        const payloadResult =
            handler.executePayload(rule.payload as LengthPayload, rule, dc({ state: "TooMuchChars" }), session);
    ...
    it("should validate to true", () => {
        const payloadResult =
            handler.executePayload(rule.payload as LengthPayload, rule, dc({ state: "Aa" }), session);
        expect(payloadResult.success).toBeTruthy();
    });
    it("should validate to true if number of chars is equal to payload length number", () => {
        const payloadResult =
            handler.executePayload(rule.payload as LengthPayload, rule, dc({ state: "AAA" }), session);
        expect(payloadResult.success).toBeTruthy();
    });
    it("should not use default error message", () => {
        ...
        const defaultErrMessage =
            `String 'AAABBB' contains more characters than ${(rule.payload as LengthPayload).length}`;
