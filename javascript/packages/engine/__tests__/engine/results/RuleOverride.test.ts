...
describe("RuleOverride", () => {
    it("should find root id", () => {
        const dataContext = (id: string, parent?: DataContext) => new DataContext(
            id, "mockName", {}, mock.contextInstanceInfo, undefined, parent
        );
    ...
    it("should extract rule override dependency from single dependencies", () => {
        ...
        const overrideDependencies = extractOverrideDependencies(
            createOverrideDependencyInfos(dependency),
            policy
        );
...
function createOverrideDependencyInfos(
    ...dependencies: { name: string, field?: string }[]
): OverrideDependencyInfo[] {
...
function createDataContext(
    dc: { name: string, field: string, value: any }
): DataContext {
