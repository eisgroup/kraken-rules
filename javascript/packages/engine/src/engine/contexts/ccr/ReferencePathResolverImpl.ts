    ...
    resolveReferencePath(origin: PathToNode, targetContextName: string): ContextReference {
        ...
        if (!paths.length) {
            throw new KrakenRuntimeError(
                ...
                "Reference target paths cannot be empty"
            );
        }
        if (paths.length === 1) {
        ...
        if (filtered.length > 1) {
            ...
            logger.error(`Failed to resolve reference path. Resolved multiple paths, '${filtered.map(p => p.path.join(".")).join(", ")}' from '${origin.join(".")}'`);
            throw new KrakenRuntimeError(
                ErrorCode.INCORRECT_MODEL_TREE,
