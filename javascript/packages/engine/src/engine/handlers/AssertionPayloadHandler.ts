...
import { AssertionPayloadResult, payloadResultCreator } from "../results/PayloadResult";
...
export class AssertionPayloadHandler implements RulePayloadHandler {
    ...
    executePayload(
        // @ts-ignore
        // TODO redesign api not to pass rule and payload
        payload: AssertionPayload, rule: Rule, dataContext: DataContext, session: ExecutionSession
        ...
        if (ExpressionEvaluationResult.isSuccess(expressionResult)) {
            return payloadResultCreator.assertion(payload, expressionResult.success, templateVariables);
        }
