...
import { TypeRef } from "../type/TypeRef";
...
export class InstanceFactory {
    ...
    private createInstance<T = unknown>(json: unknown): T {
        ...
        for (const key in json) {
            if (hasProp(json, key)) {
                ...
                if (Array.isArray(value)) { // json array
                    instance[key] = value.map(el => this.createInstance(el));
                } else if (isJson(value)) { //json object
    ...
    private initData(json: Json): any {
        if (hasMetadata(json)) {
            ...
            return new Clazz(
                json as unknown as ClassNameMarker & Json,
                this.typeRegistry
            );
        } else { // key value plain object
            return json;
        }
    }

    private createProxy<T>(json: Json & ReferenceMarker, protoSource: any): T {
        const { typeRegistry } = this;
        Object.setPrototypeOf(json, protoSource);
        const proxy = new Proxy(json, {
            get(target: unknown, prop: string): any {
