...
public class InterpretingExpressionLanguageTest {
    ...
    public void shouldEvaluatePath() {
        ...
        assertThat(eval("money", Map.of("money", Money.of(1, "USD"))), equalTo(Money.of(1, "USD")));
    ...
    public void shouldThrowIfMatchesRegExpWithNotString() {
        eval("123 matches '^[0-9]*$'");
    }

    @Test(expected = ExpressionEvaluationException.class)
    public void shouldThrowIfAccessByIndexNotNumber() {
        eval("array['0']", Map.of("array", List.of("value")));
    }

    @Test(expected = ExpressionEvaluationException.class)
    public void shouldThrowIfObjectIsNull() {
        eval("this.property", null);
    }

    @Test
    public void shouldEvaluateNullIfObjectIsMapWithoutKey() {
        assertThat(eval("map.key", Map.of("map", new HashMap<>())), nullValue());
    }

    @Test
    public void shouldEvaluateGlobalVar() {
        assertThat(eval("var", null, Map.of("var", "value")), equalTo("value"));
    }

    @Test
    public void shouldPreferLocalObjectToGlobalVar() {
        assertThat(eval("var", Map.of("var", "value1"), Map.of("var", "value2")), equalTo("value1"));
    }
    ...
    private Object eval(String expression, Object object, Map<String, Object> vars) {
        mockInvocationContext(object, vars);
        return expressionLanguage.evaluate(expression, object, vars);
    }
    ...
    private Object evalNonStrict(String expression, Object object, Map<String, Object> vars) {
        mockInvocationContext(object, vars);
        return nonStrictExpressionLanguage.evaluate(expression, object, vars);
    }

    private void mockInvocationContext(Object object, Map<String, Object> vars) {
        Map<String, Object> variables = new HashMap<>(vars);
        variables.put(TYPE_PROVIDER_PROPERTY, new TypeProvider() {
            @Override
        ...
        InvocationContextHolder.setInvocationContext(new InvocationContext(object, variables));
    }
}
