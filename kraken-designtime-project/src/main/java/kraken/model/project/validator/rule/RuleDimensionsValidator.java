 ...
 * @since 1.48.0
 */
public final class RuleDimensionsValidator implements RuleValidator {

    private final DimensionTypeCompatibilityValidator dimensionTypeCompatibilityValidator;

    public RuleDimensionsValidator(KrakenProject krakenProject) {
    ...
    public void validate(Rule rule, ValidationSession session) {
        session.addAll(dimensionTypeCompatibilityValidator.validateDimensionTypeCompatibility(rule));
    }

    @Override
    public boolean canValidate(Rule rule) {
        return rule.getMetadata() != null;
    }
